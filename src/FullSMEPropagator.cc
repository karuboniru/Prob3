...
void FullSMEPropagator::DefinePathFromLength(double LinKM, double ProdHeight)
{
    ...
    CosineZenith = (ProductionHeight * ProductionHeight - PathLength * PathLength + 2 * ProductionHeight * REarth) / (2 * PathLength * REarth);
...
double FullSMEPropagator::Calc_Prob_ee(complex<double> **ABCNDeltaE, double L)
{
    ...
    double Prob = 1. - 4. * norm((B[1] * B[0] * C[1] * C[0]) / (N[1] * N[0])) * pow(sin(DeltaE[0] * L / 2.), 2) - 4. * norm((B[2] * B[0] * C[2] * C[0]) / (N[2] * N[0])) * pow(sin(DeltaE[2] * L / 2.), 2) - 4. * norm((B[2] * B[1] * C[2] * C[1]) / (N[2] * N[1])) * pow(sin(DeltaE[1] * L / 2.), 2);
...
double FullSMEPropagator::Calc_Prob_emu(complex<double> **ABCNDeltaE, double L)
{
    ...
    double Prob = -4 * (A[1] * conj(A[0]) * B[1] * conj(B[0]) * norm(C[1] * C[0] / (N[1] * N[0]))).real() * pow(sin(DeltaE[0] * L / 2.), 2) + 2 * (A[1] * conj(A[0]) * B[1] * conj(B[0]) * norm(C[1] * C[0] / (N[1] * N[0]))).imag() * sin(DeltaE[0] * L) - 4 * (A[2] * conj(A[0]) * B[2] * conj(B[0]) * norm(C[2] * C[0] / (N[2] * N[0]))).real() * pow(sin(DeltaE[2] * L / 2.), 2) + 2 * (A[2] * conj(A[0]) * B[2] * conj(B[0]) * norm(C[2] * C[0] / (N[2] * N[0]))).imag() * sin(DeltaE[2] * L) - 4 * (A[2] * conj(A[1]) * B[2] * conj(B[1]) * norm(C[2] * C[1] / (N[2] * N[1]))).real() * pow(sin(DeltaE[1] * L / 2.), 2) + 2 * (A[2] * conj(A[1]) * B[2] * conj(B[1]) * norm(C[2] * C[1] / (N[2] * N[1]))).imag() * sin(DeltaE[1] * L);
...
double FullSMEPropagator::Calc_Prob_etau(complex<double> **ABCNDeltaE, double L)
{
    ...
    double Prob = -4 * (A[1] * conj(A[0]) * conj(C[1]) * C[0] * pow((B[1] * conj(B[0]) / (N[1] * N[0])), 2)).real() * pow(sin(DeltaE[0] * L / 2.), 2) + 2 * (A[1] * conj(A[0]) * conj(C[1]) * C[0] * pow((B[1] * conj(B[0]) / (N[1] * N[0])), 2)).imag() * sin(DeltaE[0] * L) - 4 * (A[2] * conj(A[0]) * conj(C[2]) * C[0] * pow((B[2] * conj(B[0]) / (N[2] * N[0])), 2)).real() * pow(sin(DeltaE[2] * L / 2.), 2) + 2 * (A[2] * conj(A[0]) * conj(C[2]) * C[0] * pow((B[2] * conj(B[0]) / (N[2] * N[0])), 2)).imag() * sin(DeltaE[2] * L) - 4 * (A[2] * conj(A[1]) * conj(C[2]) * C[1] * pow((B[2] * conj(B[1]) / (N[2] * N[1])), 2)).real() * pow(sin(DeltaE[1] * L / 2.), 2) + 2 * (A[2] * conj(A[1]) * conj(C[2]) * C[1] * pow((B[2] * conj(B[1]) / (N[2] * N[1])), 2)).imag() * sin(DeltaE[1] * L);
...
double FullSMEPropagator::Calc_Prob_mue(complex<double> **ABCNDeltaE, double L)
{
    ...
    double Prob = -4 * (conj(A[1]) * A[0] * conj(B[1]) * B[0] * norm(C[1] * C[0] / (N[1] * N[0]))).real() * pow(sin(DeltaE[0] * L / 2.), 2) + 2 * (conj(A[1]) * A[0] * conj(B[1]) * B[0] * norm(C[1] * C[0] / (N[1] * N[0]))).imag() * sin(DeltaE[0] * L) - 4 * (conj(A[2]) * A[0] * conj(B[2]) * B[0] * norm(C[2] * C[0] / (N[2] * N[0]))).real() * pow(sin(DeltaE[2] * L / 2.), 2) + 2 * (conj(A[2]) * A[0] * conj(B[2]) * B[0] * norm(C[2] * C[0] / (N[2] * N[0]))).imag() * sin(DeltaE[2] * L) - 4 * (conj(A[2]) * A[1] * conj(B[2]) * B[1] * norm(C[2] * C[1] / (N[2] * N[1]))).real() * pow(sin(DeltaE[1] * L / 2.), 2) + 2 * (conj(A[2]) * A[1] * conj(B[2]) * B[1] * norm(C[2] * C[1] / (N[2] * N[1]))).imag() * sin(DeltaE[1] * L);
...
double FullSMEPropagator::Calc_Prob_mumu(complex<double> **ABCNDeltaE, double L)
{
    ...
    double Prob = 1. - 4. * norm((A[1] * A[0] * C[1] * C[0]) / (N[1] * N[0])) * pow(sin(DeltaE[0] * L / 2.), 2) - 4. * norm((A[2] * A[0] * C[2] * C[0]) / (N[2] * N[0])) * pow(sin(DeltaE[2] * L / 2.), 2) - 4. * norm((A[2] * A[1] * C[2] * C[1]) / (N[2] * N[1])) * pow(sin(DeltaE[1] * L / 2.), 2);
...
double FullSMEPropagator::Calc_Prob_mutau(complex<double> **ABCNDeltaE, double L)
{
    ...
    double Prob = -4 * (B[1] * conj(B[0]) * conj(C[1]) * C[0] * norm(A[1] * A[0] / (N[1] * N[0]))).real() * pow(sin(DeltaE[0] * L / 2.), 2) + 2 * (B[1] * conj(B[0]) * conj(C[1]) * C[0] * norm(A[1] * A[0] / (N[1] * N[0]))).imag() * sin(DeltaE[0] * L) - 4 * (B[2] * conj(B[0]) * conj(C[2]) * C[0] * norm(A[2] * A[0] / (N[2] * N[0]))).real() * pow(sin(DeltaE[2] * L / 2.), 2) + 2 * (B[2] * conj(B[0]) * conj(C[2]) * C[0] * norm(A[2] * A[0] / (N[2] * N[0]))).imag() * sin(DeltaE[2] * L) - 4 * (B[2] * conj(B[1]) * conj(C[2]) * C[1] * norm(A[2] * A[1] / (N[2] * N[1]))).real() * pow(sin(DeltaE[1] * L / 2.), 2) + 2 * (B[2] * conj(B[1]) * conj(C[2]) * C[1] * norm(A[2] * A[1] / (N[2] * N[1]))).imag() * sin(DeltaE[1] * L);
...
double FullSMEPropagator::Calc_Prob_taue(complex<double> **ABCNDeltaE, double L)
{
    ...
    double Prob = -4 * (conj(A[1]) * A[0] * C[1] * conj(C[0]) * pow(((conj(B[1]) * B[0]) / (N[1] * N[0])), 2)).real() * pow(sin(DeltaE[0] * L / 2.), 2) + 2 * (conj(A[1]) * A[0] * C[1] * conj(C[0]) * pow(((conj(B[1]) * B[0]) / (N[1] * N[0])), 2)).imag() * sin(DeltaE[0] * L) - 4 * (conj(A[2]) * A[0] * C[2] * conj(C[0]) * pow(((conj(B[2]) * B[0]) / (N[2] * N[0])), 2)).real() * pow(sin(DeltaE[2] * L / 2.), 2) + 2 * (conj(A[2]) * A[0] * C[2] * conj(C[0]) * pow(((conj(B[2]) * B[0]) / (N[2] * N[0])), 2)).imag() * sin(DeltaE[2] * L) - 4 * (conj(A[2]) * A[1] * C[2] * conj(C[1]) * pow(((conj(B[2]) * B[1]) / (N[2] * N[1])), 2)).real() * pow(sin(DeltaE[1] * L / 2.), 2) + 2 * (conj(A[2]) * A[1] * C[2] * conj(C[1]) * pow(((conj(B[2]) * B[1]) / (N[2] * N[1])), 2)).imag() * sin(DeltaE[1] * L);
...
double FullSMEPropagator::Calc_Prob_taumu(complex<double> **ABCNDeltaE, double L)
{
    ...
    double Prob = -4 * (conj(B[1]) * B[0] * C[1] * conj(C[0]) * norm(A[1] * A[0] / (N[1] * N[0]))).real() * pow(sin(DeltaE[0] * L / 2.), 2) + 2 * (conj(B[1]) * B[0] * C[1] * conj(C[0]) * norm(A[1] * A[0] / (N[1] * N[0]))).imag() * sin(DeltaE[0] * L) - 4 * (conj(B[2]) * B[0] * C[2] * conj(C[0]) * norm(A[2] * A[0] / (N[2] * N[0]))).real() * pow(sin(DeltaE[2] * L / 2.), 2) + 2 * (conj(B[2]) * B[0] * C[2] * conj(C[0]) * norm(A[2] * A[0] / (N[2] * N[0]))).imag() * sin(DeltaE[2] * L) - 4 * (conj(B[2]) * B[1] * C[2] * conj(C[1]) * norm(A[2] * A[1] / (N[2] * N[1]))).real() * pow(sin(DeltaE[1] * L / 2.), 2) + 2 * (conj(B[2]) * B[1] * C[2] * conj(C[1]) * norm(A[2] * A[1] / (N[2] * N[1]))).imag() * sin(DeltaE[1] * L);
...
double FullSMEPropagator::Calc_Prob_tautau(complex<double> **ABCNDeltaE, double L)
{
    ...
    double Prob = 1. - 4. * norm((A[1] * A[0] * B[1] * B[0]) / (N[1] * N[0])) * pow(sin(DeltaE[0] * L / 2.), 2) - 4. * norm((A[2] * A[0] * B[2] * B[0]) / (N[2] * N[0])) * pow(sin(DeltaE[2] * L / 2.), 2) - 4. * norm((A[2] * A[1] * B[2] * B[1]) / (N[2] * N[1])) * pow(sin(DeltaE[1] * L / 2.), 2);
...
void FullSMEPropagator::SetLVMatrixEntry(const char *matrix, int i, int j, double re, double im)
{
    ...
    if (strcmp(matrix, "C") == 0)
        fCLV[i][j] = complex<double>(re, im);
    else if (strcmp(matrix, "A") == 0)
        fALV[i][j] = complex<double>(re, im);
    else
    {
                                                     ...
                                                     vector<complex<double>> MatterEffects)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        array2D[3][i] = sqrt(norm(array2D[0][i] * array2D[1][i])            //Ni
                             + norm(array2D[0][i] * array2D[2][i]) + norm(array2D[1][i] * array2D[2][i]));
...
void FullSMEPropagator::SetLVUPMNSmatrix(double ssq12, double ssq23, double ssq13, double dCP)
{
    ...
    double th12 = asin(sqrt(ssq12));
    double c12 = cos(th12);
    double s12 = sin(th12);
    double th23 = asin(sqrt(ssq23));
    double c23 = cos(th23);
    double s23 = sin(th23);
    double th13 = asin(sqrt(ssq13));
    double c13 = cos(th13);
    double s13 = sin(th13);
