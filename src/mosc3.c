 ...
 */
void init_mixing_matrix(dm21f, dm32f, s12f, s23f, s31f, dcpf) double dm21f, dm32f, s12f, s23f, s31f, dcpf;
{
    dm21 = dm21f;
    dm32 = dm32f;
    s12 = s12f;
 ...
 *   Initialize mixing matrix, etc.
 */
void init_mass_with_mixing_matrix(dm21f, dm32f, lMNS) double dm21f, dm32f;
double lMNS[][3][2];
{
...
void get_oscillation_parameters(dm21f, dm32f, s12f, s23f, s31f, dcpf) double dm21f, dm32f, s12f, s23f, s31f, dcpf;
{
    dm21f = dm21;
    dm32f = dm32;
    s12f = s12;
 ...
 */
void get_wavelength_23(energy, lambda23) double energy;
double *lambda23;
{
 ...
 */
void get_mixing_matrix_real(mixtmp) double mixtmp[3][3];
{
    int i, j;
    for (j = 0; j < 3; j++)
    {
 ...
 */
void get_transition_matrix(nutypei, Enuf, rhof, Lenf, Aout, phase_offsetf) int nutypei;
 ...
 *        C = A X B
 */
void multiply_complex_matrix(A, B, C) double A[][3][2];
double B[][3][2];
double C[][3][2];
{
    int i, j, k;
 ...
 *        W = A X V
 */
void multiply_complex_matvec(A, V, W) double A[][3][2];
double V[][2];
double W[][2];
{
    int i;
 ...
 *        A --> B
 */
void copy_complex_matrix(A, B) double A[][3][2];
double B[][3][2];
{
 ...
 *        
 */
void clear_complex_matrix(A) double A[][3][2];
{
 ...
 *
 */
void get_vacuum_probability(nutype, energy, path, prob) int nutype;
double energy, path;
double prob[][3];
{
    double lovere;
...
void convert_from_mass_eigenstate(state, flavor, pure) int state;
int flavor;
double pure[][2];
{
    int i, j;
