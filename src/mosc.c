...
{
    if (flavor == nue_type)
        matterFlavor = nue_type;
    else if (flavor == sterile_type)
        matterFlavor = sterile_type;
    else
    {
          ...
          double dmMatMat[][3], double dmMatVac[][3])
{
    ...
    if (matterFlavor == nue_type)
    {
        ...
        if (antitype < 0)
            fac = tworttwoGf * Enu * rho; /* Anti-neutrinos */
        else
            fac = -tworttwoGf * Enu * rho; /* Real-neutrinos */
    }
    else if (matterFlavor == sterile_type)
    {
        /* If we're doing matter effects for sterile neutrinos */
        if (antitype < 0)
            fac = -0.5 * tworttwoGf * Enu * rho; /* Anti-neutrinos */
        else
            fac = 0.5 * tworttwoGf * Enu * rho; /* Real-neutrinos */
    }
                 ...
                 int antitype, double product[][3][3][2])
{
    ...
    if (matterFlavor == nue_type)
    {
        /* If we're doing matter effects for electron neutrinos */
        if (antitype < 0)
            fac = tworttwoGf * E * rho; /* Anti-neutrinos */
        else
            fac = -tworttwoGf * E * rho; /* Real-neutrinos */
    }
    else if (matterFlavor == sterile_type)
    {
        /* If we're doing matter effects for sterile neutrinos */
        if (antitype < 0)
            fac = -0.5 * tworttwoGf * E * rho; /* Anti-neutrinos */
        else
            fac = 0.5 * tworttwoGf * E * rho; /* Real-neutrinos */
    }
...
void setmix_sin(s12, s23, s13, dcp, Mix) double s12, s23, s13, dcp;
double Mix[][3][2];
{
...
void matrix2mix(matrix, mix) double *matrix;
double mix[][3][2];
{
    int i, j;
